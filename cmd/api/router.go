...
func (app *application) Router() *michi.Router {
	...
	rateLimiter := NewRateLimiter(RateLimiterConfig{
		Skipper: func(r *http.Request) bool {
			// Define logic to skip rate limiting for certain requests
		...
		IdentifierExtractor: func(r *http.Request) (string, error) {
			return r.RemoteAddr, nil // Use RemoteAddr or extract IP more accurately
		},
	...
	r.Route("/", func(sub *michi.Router) {
		...
		sub.HandleFunc("POST login", http.HandlerFunc((app.SigninHandler))) // In your router setup
		...
		sub.HandleFunc("GET preproject/{id}", app.AuthMiddleware(http.HandlerFunc(app.GetPreProjectsHandlerByID)))
		...
		sub.HandleFunc("GET conversation/{conversation_id}", app.AuthMiddleware(app.ChatParticipantMiddleware(http.HandlerFunc(app.GetChatsHandler)))) // Get conversation between two users
		...
		sub.HandleFunc("GET conversations", app.AuthMiddleware(http.HandlerFunc(app.GetConversationsHandler)))                                         // Get all chats
