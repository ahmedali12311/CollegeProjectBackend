...
func (app *application) SignupHandler(w http.ResponseWriter, r *http.Request) {
	...
	if !isAdmin {
		role = 3 // Default role for non-admins
	} else {
		if roleStr == "" {
			role = 3 // Default role for admins if not specified
		} else {
			var err error
...
func (app *application) GetNumderOfStudents(w http.ResponseWriter, r *http.Request) {
	...
	CountGraduationStudents, err := app.Model.UserRoleDB.CountGraduationStudents()
	if err != nil {
		app.serverErrorResponse(w, r, err)
		return
	}
