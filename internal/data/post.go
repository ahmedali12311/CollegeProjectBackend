...
func ValidatePost(v *validator.Validator, post *Post, fields ...string) {
	for _, field := range fields {
		switch field {
		case "description":
			v.Check(post.Description != "", "description", "Post description is required")
			v.Check(len(post.Description) <= 4000, "description", "Description must be less than 4000 characters")
		}
	}
}
